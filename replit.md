...
- **`artifacts/videosave`** — React + Vite + Tailwind v4 frontend (mounted at `/`).
- **`artifacts/api-server`** — Express + Drizzle backend (mounted at `/api`). Talks to PostgreSQL and shells out to `yt-dlp` for video extraction.
- **`artifacts/mockup-sandbox`** — Component preview environment for design exploration (mounted at `/__mockup`).
...
Run `pnpm --filter @workspace/api-spec run codegen` after editing the OpenAPI file to regenerate the zod and client packages.
...
- `pnpm --filter @workspace/db run push --force` — apply schema changes to DB.
- `pnpm --filter @workspace/scripts run seed` — seed the 4 starter blog posts.
- `pnpm --filter @workspace/videosave run typecheck` — type-check the frontend.
- `pnpm --filter @workspace/api-server run typecheck` — type-check the backend.
